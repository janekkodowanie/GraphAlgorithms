...
def lexBFS(Graph):
    ...
    while len(visited) < len(Graph) - 1:
        ...
        while idx < len(vertices):
            i = 0
            # partitions in vertices based on their
            # adjacency to the currently selected vertex u
            ...
            # Set of vertices is split into two sets,
            # therefore is removed from the list of vertices
            vertices.remove(vertices[idx])
            idx += 1

        new_vertices = []

        # intersection of the visited vertices and the neighbors of u
        intersection = set()
        ...
        # The parent is the last visited vertex
        # whose set of neighbors intersects with
        # the RN set of u. If such a vertex is found,
        # it is assigned as the parent of u.
        found = False
